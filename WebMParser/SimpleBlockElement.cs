namespace SpawnDev.WebMParser
{
    public class SimpleBlockElement : BinaryElement
    {
        ...
        public override string ToString() => $"{Index} {Id} - IdChain: [ {string.Join(" ", IdChain.ToArray())} ] Type: {this.GetType().Name} Length: {Length} bytes TrackId: {TrackId} Timecode: {Timecode}";
    }
}
